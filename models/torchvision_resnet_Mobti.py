...
class ResNet(nn.Module):
    ...
    def feature_list(self, x):
        out_list = []
        out = self.maxpool(self.relu(self.bn1(self.my_conv1(x))))
        out_list.append(out)
        out = self.layer1(out)
        out_list.append(out)
        out = self.layer2(out)
    ...
    def gen_forward(self, x):
        out, penultimate = self.penultimate_forward(x)
        out = torch.sigmoid(out)                       #### out means g(x)
        penultimate = torch.flatten(self.avgpool(penultimate), 1)
...
def resnet152(num_c, pretrained=False, progress=True, **kwargs):
    ...
    Args:
        ...
        progress (bool): If True, displays a progress bar of the download to stderr
    """
    return _resnet('resnet152', Bottleneck, [3, 8, 36, 3], num_c, pretrained, progress,
                   **kwargs)


def resnext50_32x4d(num_c, pretrained=False, progress=True, **kwargs):
    ...
    return _resnet('resnext50_32x4d', Bottleneck, [3, 4, 6, 3],num_c,
                   pretrained, progress, **kwargs)


def resnext101_32x8d(num_c, pretrained=False, progress=True, **kwargs):
    ...
    return _resnet('resnext101_32x8d', Bottleneck, [3, 4, 23, 3],num_c,
                   pretrained, progress, **kwargs)


def wide_resnet50_2(num_c, pretrained=False, progress=True, **kwargs):
    ...
    return _resnet('wide_resnet50_2', Bottleneck, [3, 4, 6, 3],num_c,
                   pretrained, progress, **kwargs)


def wide_resnet101_2(num_c, pretrained=False, progress=True, **kwargs):
    ...
    """
    kwargs['width_per_group'] = 64 * 2
    return _resnet('wide_resnet101_2', Bottleneck, [3, 4, 23, 3],num_c,
                   pretrained, progress, **kwargs)

if __name__ == '__main__':
    model = resnet101(num_c = 5, pretrained = False).to("cuda")
